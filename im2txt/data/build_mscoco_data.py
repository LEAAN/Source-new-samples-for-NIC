...
# --train_image_dir=/mnt/raid/data/ni/dnn/zlian/mscoco/raw-data/train2014/
# --val_image_dir=/mnt/raid/data/ni/dnn/zlian/mscoco/raw-data/val2014/
# --train_captions_file=/mnt/raid/data/ni/dnn/zlian/mscoco/raw-data/annotations/captions_train2014.json
# --val_captions_file=/mnt/raid/data/ni/dnn/zlian/mscoco/raw-data/annotations/captions_val2014.json
# --output_dir=/mnt/raid/data/ni/dnn/zlian/mscoco
...
def _create_vocab(captions):
  ...
  def recreate_vocabulary():
    '''
    Either build tfrecords from raw mscoco images, and train from scratch,
    or rebuild tfrecords using the word count file provided by other github users, so that the word indexing in tf records
    is consistant with the ckpt of the same github users.
    If using model ckpt of some other people, while creating tfrecords (of mscoco data) of your own, the word indexing
    might see mismatch, resulting in wrong labels seen by the model.
    '''
    print ('Recreate vocabulary from existing word_counts.txt')
    word_counts_file_path='/mnt/raid/data/ni/dnn/zlian/word_counts.txt'
    with tf.gfile.FastGFile(word_counts_file_path, 'r') as f:
      for line in f.readlines():
        line=line.split(' ')
        try:
          word_counts.append((unicode(line[0][2:-1]), int(line[1][:-1])))
        except Exception as e:
          print (e)
          print (line)
          word_counts.append((unicode(line[0][2:-1]), int(line[1])))
    return word_counts

  # Uncomment this when using existing word_counts.txt to build tfrecords
  # word_counts=recreate_vocabulary()
